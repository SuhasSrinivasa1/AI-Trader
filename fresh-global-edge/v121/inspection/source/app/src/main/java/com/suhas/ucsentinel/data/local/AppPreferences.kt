...
class AppPreferences(private val context:Context){
    ...
    private fun updateShadowLearning(record:TradeAutopsyRecord){
        ...
        for(x in record.shadowResults){
            val key="${record.engineLabel}|${record.regime.name}|${x.strategyId}";val j=root.optJSONObject(key)?:JSONObject()
            j.put("strategyName",x.strategyName).put("observations",j.optInt("observations")+1)
                .put("tradedWins",j.optInt("tradedWins")+(if(x.outcome==ShadowOutcome.WIN)1 else 0))
                .put("tradedLosses",j.optInt("tradedLosses")+(if(x.outcome==ShadowOutcome.LOSS)1 else 0))
                .put("avoidedLosses",j.optInt("avoidedLosses")+(if(x.outcome==ShadowOutcome.AVOIDED_LOSS)1 else 0))
                .put("missedWins",j.optInt("missedWins")+(if(x.outcome==ShadowOutcome.MISSED_WIN)1 else 0))
                .put("sumReturn",j.optDouble("sumReturn")+x.returnPct).put("lastAt",record.generatedAt)
            val days=j.optString("days").split(',').filter{it.isNotBlank()}.toMutableSet();days+=record.sessionDate;j.put("days",days.sorted().takeLast(30).joinToString(","));root.put(key,j)
        }
    ...
    fun updateStrategyResult(strategyId:String,name:String,returnPct:Double,win:Boolean){val safeReturn=finite(returnPct);val root=runCatching{JSONObject(prefs.getString("strategy_stats","{}")?:"{}")}.getOrElse{JSONObject()};val x=root.optJSONObject(strategyId)?:JSONObject();val n=x.optInt("observations")+1;val wins=x.optInt("wins")+(if(win)1 else 0);val sum=x.optDouble("sumReturn")+safeReturn;val equity=x.optDouble("equity",0.0)+safeReturn;val peak=maxOf(x.optDouble("peak",0.0),equity);val dd=minOf(x.optDouble("maxDrawdown",0.0),equity-peak);x.put("name",name).put("observations",n).put("wins",wins).put("sumReturn",sum).put("equity",equity).put("peak",peak).put("maxDrawdown",dd);root.put(strategyId,x);prefs.edit().putString("strategy_stats",root.toString()).apply()}
    fun strategyPerformances(defs:List<TradingStrategyDefinition>,settings:AppSettings):List<StrategyPerformance>{val root=runCatching{JSONObject(prefs.getString("strategy_stats","{}")?:"{}")}.getOrElse{JSONObject()};return defs.map{d->val x=root.optJSONObject(d.id)?:JSONObject();val n=x.optInt("observations");val w=x.optInt("wins");val acc=if(n==0)0.0 else w*100.0/n;val avg=if(n==0)0.0 else x.optDouble("sumReturn")/n;val floor=wilsonLower(w,n)*100.0;val status=when{n>=settings.strategyMinChampionSamples&&acc>=settings.strategyMinChampionAccuracy&&avg>0&&floor>=50.0->StrategyStatus.CHAMPION;n>=10&&avg<=0->StrategyStatus.PROBATION;n>=5->StrategyStatus.ACTIVE;else->StrategyStatus.CHALLENGER};StrategyPerformance(d.id,d.name,n,w,acc,avg,avg,kotlin.math.abs(x.optDouble("maxDrawdown")),floor,status)}.sortedWith(compareBy<StrategyPerformance>{it.status.ordinal}.thenByDescending{it.expectancyPct}.thenByDescending{it.accuracyPct})}
    ...
    fun updateGlobalLearning(key:String,returnPct:Double,win:Boolean){
        ...
        val root=globalLearningRoot();val x=root.optJSONObject(key)?:JSONObject()
        x.put("observations",x.optInt("observations")+1)
            .put("wins",x.optInt("wins")+(if(win)1 else 0))
            .put("sumReturn",x.optDouble("sumReturn")+returnPct)
            .put("lastAt",System.currentTimeMillis())
        root.put(key,x);prefs.edit().putString("global_learning_stats",root.toString()).apply()
    }
    fun globalScoreAdjustment(key:String):Double{
        val x=globalLearningRoot().optJSONObject(key)?:return 0.0
        val n=x.optInt("observations");if(n<4)return 0.0
        val wins=x.optInt("wins");val winRate=wins.toDouble()/n
        val avg=x.optDouble("sumReturn")/n
        return (((winRate-0.5)*10.0)+(avg*1.5)).coerceIn(-6.0,6.0)
    }
