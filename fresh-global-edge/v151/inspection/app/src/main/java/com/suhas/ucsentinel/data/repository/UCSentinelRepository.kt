...
class GlobalEdgeAITraderRepository(context:Context){
    ...
    suspend fun placeManualMarketOrder(symbol:String,side:String,product:String,quantity:Int,expectedEntryPrice:Double=0.0):String{
    ...
    private fun governedStrategyPerformances(defs:List<TradingStrategyDefinition>,settings:AppSettings):List<StrategyPerformance>{
    ...
    private fun activeStrategyDefinitions(settings:AppSettings):Pair<StrategyCatalogClient.Bundle,List<TradingStrategyDefinition>>{
        ...
        val thisWeek=weekKey(today);val perfs=governedStrategyPerformances(bundle.strategies,settings).associateBy{it.strategyId}
    ...
    private fun nextTradingDate(from:LocalDate):LocalDate=NseTradingCalendar.nextTradingDay(from)
    ...
    suspend fun scanTradingStrategies(progress:suspend(String)->Unit={}):StrategyTournamentSummary=strategyMutex.withLock{
        ...
        val champions=perfs.count{it.status==StrategyStatus.CHAMPION}
