...
class GrowwClient {
    ...
    private fun parseQuote(symbol: String, p: JSONObject): Quote {
        ...
        fun levels(name: String): List<DepthLevel> {
            ...
            return buildList {
                for (i in 0 until arr.length()) {
                    ...
                    add(DepthLevel(x.optDouble("price"), x.optLong("quantity")))
                }
            }
        }
        ...
        return Quote(
            symbol = symbol,
            lastPrice = p.optDouble("last_price"),
            previousClose = o.optDouble("close"),
            dayChangePercent = p.optDouble("day_change_perc"),
            upperCircuit = p.optDouble("upper_circuit_limit"),
            lowerCircuit = p.optDouble("lower_circuit_limit"),
            volume = p.optLong("volume"),
            totalBuyQuantity = p.optLong("total_buy_quantity"),
            totalSellQuantity = p.optLong("total_sell_quantity"),
            bidPrice = p.optDouble("bid_price"),
            bidQuantity = p.optLong("bid_quantity"),
            offerPrice = p.optDouble("offer_price"),
            offerQuantity = p.optLong("offer_quantity"),
            marketCap = p.optDouble("market_cap"),
            week52High = p.optDouble("week_52_high"),
            week52Low = p.optDouble("week_52_low"),
            ohlc = Ohlc(
