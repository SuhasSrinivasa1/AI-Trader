...
class GlobalEdgeAITraderRepository(context:Context){
    ...
    private fun recordCandidateCalls(section:ScannerSection,bucket:TradeCallBucket,candidates:List<Candidate>,source:String,nowMs:Long=System.currentTimeMillis()):Int{
        ...
        for(c in candidates){
            ...
            val detail="${c.companyName} • score ${"%.0f".format(c.score)} • ${"%.2f".format(c.dayChangePercent)}% • vol ${"%.1f".format(c.volumeRatio)}x"
    ...
    suspend fun reconcileTradeCallLedger():Int{
        val all=prefs.loadTradeCalls(1500).toMutableList();val open=all.filter{it.outcome==TradeCallOutcome.OPEN}
        if(open.isEmpty())return 0
        if(!ensureAutomationAuthentication())return 0
        val token=accessToken();val now=ZonedDateTime.now(ist);val today=now.toLocalDate();var changed=0
        val quoteCache=mutableMapOf<String,Quote?>();val candleCache=mutableMapOf<String,List<Candle>?>()
        val updated=all.map{call->
            ...
            if(outcome==null&&today==targetDate&&q!=null){
                ...
                outcome=when{stopHit->TradeCallOutcome.LOSS to call.stopPrice;targetHit->TradeCallOutcome.WIN to call.targetPrice;else->null}
            }
            val expired=today>targetDate||(today==targetDate&&now.toLocalTime()>=LocalTime.of(15,30))
            if(outcome==null&&expired){
                // Do not guess if the historical request failed; retry on the next learning pass.
                if(candles==null)return@map call
                val last=candles.lastOrNull()?.close?:q?.lastPrice?:call.entryPrice
                outcome=TradeCallOutcome.LOSS to last
            }
            if(outcome==null)return@map call
            val(out,exit)=outcome!!;val ret=callReturnPct(call,exit);changed++
            if(call.engine==TradeCallEngine.GLOBAL&&call.learningKey.isNotBlank())prefs.updateGlobalLearning(call.learningKey,ret,out==TradeCallOutcome.WIN)
            call.copy(outcome=out,closedAt=System.currentTimeMillis(),exitPrice=exit,returnPct=ret,closeReason=when(out){TradeCallOutcome.WIN->"Target reached";TradeCallOutcome.LOSS->if(expired)"Target not reached by session close" else "Stop reached";else->""})
        }
        if(changed>0){prefs.saveTradeCalls(updated);DiagnosticLog.log(appContext,"CALL-LEDGER","closed $changed calls with WIN/LOSS")}
        return changed
    }

    suspend fun closeExpiredStrategyCalls():Int{
        val now=ZonedDateTime.now(ist);if(now.toLocalTime()<LocalTime.of(15,30)&&marketSessionInfo(now).isOpen)return 0
        val live=prefs.loadStrategyLive();if(live.isEmpty())return 0
        if(!ensureAutomationAuthentication())return 0
        val token=accessToken();val closed=prefs.loadStrategyClosed(500).toMutableList();val surviving=mutableListOf<StrategyRecommendation>();var changed=0
        for(r in live){
            val opened=Instant.ofEpochMilli(r.openedAt).atZone(ist);val sessionDate=opened.toLocalDate()
            if(now.toLocalDate()<sessionDate||(now.toLocalDate()==sessionDate&&now.toLocalTime()<LocalTime.of(15,30))){surviving+=r;continue}
            val end=sessionDate.atTime(15,31).atZone(ist);val candles=runCatching{groww.getHistoricalCandles(token,r.setup.symbol,opened.format(dateTimeFmt),end.format(dateTimeFmt),"5minute")}.getOrNull()
            if(candles==null){surviving+=r;continue}
            ...
            var win=false;var stopHit=false;var exit=candles.lastOrNull()?.close?:r.lastPrice
            for(c in candles.sortedBy{it.epochSeconds}){
                ...
                if(th&&sh){stopHit=true;exit=stop;break}
                if(sh){stopHit=true;exit=stop;break}
                if(th){win=true;exit=target;break}
            }
            val ret=if(r.setup.entryPrice<=0.0)0.0 else if(r.setup.direction==TradeDirection.LONG)(exit/r.setup.entryPrice-1.0)*100.0 else (r.setup.entryPrice/exit-1.0)*100.0
            val done=r.copy(lastSeenAt=System.currentTimeMillis(),lastPrice=exit,closedAt=System.currentTimeMillis(),exitPrice=exit,status=if(win)StrategyRecommendationStatus.WIN else StrategyRecommendationStatus.LOSS,returnPct=ret,closeReason=if(win)"Target reached" else if(stopHit)"Stop reached" else "Target not reached by session close")
            closed.removeAll{it.id==done.id};closed.add(done);prefs.updateStrategyResult(r.setup.strategyId,r.setup.strategyName,ret,win);changed++
        }
    ...
    suspend fun scanUpperCircuitNextSession(progress:suspend(String)->Unit={}):ScanSummary=scanMutex.withLock{
        ...
        recordCandidateCalls(ScannerSection.UC_CONTINUATION,TradeCallBucket.NEXT_SESSION,uc.candidates,"UC next-session research",System.currentTimeMillis())
    ...
    private fun maybeAdaptDailySettings(){
        ...
        val tuned=current.copy(minScore=(ucBase+ucAutopsy+ucCalibration).coerceIn(66.0,88.0),demandMinScore=(prBase+prAutopsy+prCalibration).coerceIn(62.0,86.0))
