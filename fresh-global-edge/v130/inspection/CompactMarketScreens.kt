    ...
    LazyColumn(Modifier.fillMaxSize().padding(padding).padding(horizontal=14.dp),verticalArrangement=Arrangement.spacedBy(10.dp),contentPadding=PaddingValues(top=12.dp,bottom=24.dp)){
        ...
        when(view){
            ...
            UcCompactView.LIVE->{
                ...
                else items(live,key={it.symbol}){c->TradeCard(c.symbol,"LONG • PRE-UC",c.score,ucPlan(c),"${c.companyName} • ${"%.2f".format(c.dayChangePercent)}% today • ${"%.1f".format(c.volumeRatio)}x volume","LIVE • tap MODEL score for ₹20,000 CNC broker handoff",orderable=true)}
            }
    ...
    LazyColumn(Modifier.fillMaxSize().padding(padding).padding(horizontal=14.dp),verticalArrangement=Arrangement.spacedBy(10.dp),contentPadding=PaddingValues(top=12.dp,bottom=24.dp)){
        ...
        when(view){
            ...
            GlobalCompactView.LIVE->{
                ...
                else items(live,key={"live-${it.direction}-${it.indianSymbol}"}){c->TradeCard(c.indianSymbol,c.direction.name,c.score,globalPlan(c),"Lead ${c.foreignTicker} • ${c.exchange} • foreign ${"%.2f".format(c.foreignDayPct)}% • India ${"%.2f".format(c.indianFromOpenPct)}%","LIVE • ${c.action.name.replace('_',' ')} • tap MODEL score to prepare order",orderable=true)}
            }
