...
public class AutomationService extends Service {
    ...
    public static final String ACTION_MULTIFY_EXIT="com.ps.shadowtrader.MULTIFY_EXIT";
    ...
    private volatile boolean labBusy=false;
    ...
    @Override public int onStartCommand(Intent i,int flags,int startId){
        ...
        if(i!=null&&ACTION_MULTIFY_EXIT.equals(i.getAction())){
            String symbol=i.getStringExtra("symbol");
            if(symbol!=null&&!symbol.isEmpty())
                getSharedPreferences("state",MODE_PRIVATE).edit()
                        .putString("pending_multify_exit_symbol",symbol)
                        .putLong("pending_multify_exit_ts",System.currentTimeMillis()).apply();
        }
    ...
    private void cycle(){
        ...
        state.edit().putLong("service_heartbeat",System.currentTimeMillis()).apply();
    ...
    private String ensureToken(android.content.SharedPreferences state){
        ...
        String token=secure.get("token"),tt=secure.get("totp_token"),secret=secure.get("totp_secret");
        long lastAuth=state.getLong("last_auth_check",0);
        boolean hasTotp=!tt.isEmpty()&&!secret.isEmpty();
        boolean dailyDue=hasTotp&&state.getLong("last_daily_auth_day",-1)!=dayKey();
        boolean validationDue=token.isEmpty()||now-lastAuth>45*60000L;

        // Once per IST calendar day, deliberately mint a fresh Groww access token from TOTP.
        if(dailyDue){
            try{
                String code=TotpUtil.now(secret);
                String fresh=api.generateAccessTokenTotp(tt,code);
                api.user(fresh);secure.put("token",fresh);
                state.edit().putLong("last_auth_check",now).putLong("last_daily_auth_day",dayKey())
                        .putBoolean("token_ok",true).putString("engine_status","Ready")
                        .putString("auth_mode","Automatic TOTP").apply();
                new EventStore(this).add("AUTH","Daily Groww authentication refreshed automatically");
                return fresh;
            }catch(Exception e){
                state.edit().putBoolean("token_ok",false).putString("engine_status","Authentication attention").apply();
                new EventStore(this).add("AUTH","Daily TOTP authentication failed: "+shortMsg(e));
                return "";
            }
        }

        if(!validationDue)return token;

        if(!token.isEmpty()){
            try{
                api.user(token);
                state.edit().putLong("last_auth_check",now).putBoolean("token_ok",true)
                        .putString("engine_status","Ready")
                        .putString("auth_mode","Automatic TOTP").apply();
                return token;
        ...
        state.edit().putBoolean("token_ok",false).putString("engine_status","Authentication required").apply();
        return "";
    }


    private void processPendingMultifyExit(String token,android.content.SharedPreferences state,long now){
        String symbol=state.getString("pending_multify_exit_symbol","");
        long eventTs=state.getLong("pending_multify_exit_ts",0);
        long handled=state.getLong("handled_multify_exit_ts",0);
        if(symbol.isEmpty()||eventTs<=handled)return;

        TradeStateStore store=new TradeStateStore(this);
        TradeState ts=store.load();
        String result;
        if(ts.side!=Side.FLAT&&symbol.equalsIgnoreCase(ts.symbol)){
            try{
                Double px=new GrowwClient().ltps(token,Collections.singletonList(symbol)).get(symbol);
                if(px==null||Double.isNaN(px))throw new IllegalStateException("LTP unavailable");
                double grossPct=ts.entry<=0?0:(ts.side==Side.LONG?(px-ts.entry)/ts.entry:(ts.entry-px)/ts.entry);
                double approxNetPct=grossPct-(AdaptiveTradeController.COST_RATE_PER_ORDER*2);
                String label=approxNetPct>0?"WIN":"LOSS";
                String action=new AdaptiveTradeController(new AdaptiveModel(this))
                        .forceExit(ts,px,"Multify explicit exit / book-profit notification");
                store.save(ts);
                result=label+" • "+action+" • approx net "+String.format(Locale.US,"%+.2f%%",approxNetPct*100);
            }catch(Exception e){
                result="Multify EXIT "+symbol+" captured but shadow close retry needed: "+shortMsg(e);
            }
        }else{
            result="Multify EXIT "+symbol+" captured • no matching open shadow position";
        }

        state.edit().putLong("handled_multify_exit_ts",eventTs)
                .putString("last_action",result)
                .putString("last_multify_exit_result",result).apply();
        new EventStore(this).add("MULTIFY",result);
    }
    ...
    private void runAdaptiveMarket(String token,android.content.SharedPreferences state,long now,int hm){
        ...
        try{
            ...
            AdaptiveModel model=new AdaptiveModel(this);
            if(ts.side!=Side.FLAT){
                Double observed=prices.get(ts.symbol);
                if(observed!=null&&!Double.isNaN(observed))model.observeOpen(ts.symbol,observed);
            }
    ...
    private void maybeLaunchLab(String token,android.content.SharedPreferences state,long now,boolean market){
        if(labBusy)return;
        ...
        if(market){
            TradeState ts=new TradeStateStore(this).load();
            boolean busyWithIntraday=ts.side!=Side.FLAT||!new WatchlistStore(this).symbols().isEmpty();
            if(busyWithIntraday)return;
        }

        labBusy=true;
        state.edit().putLong("last_lab_ts",now)
                .putString("last_lab_status",first?"Starting first 30-day universe batch":"30-day scan queued").apply();

        new Thread(()->{
            try{
                String x=new MomentumLabEngine(this,new GrowwClient()).scanNextBatch(token);
                getSharedPreferences("state",MODE_PRIVATE).edit()
                        .putLong("last_lab_complete_ts",System.currentTimeMillis())
                        .putString("last_lab_status",x).apply();
            }catch(Exception e){
                String msg="Scan retry: "+shortMsg(e);
                getSharedPreferences("state",MODE_PRIVATE).edit()
                        .putString("last_lab_status",msg).apply();
                new EventStore(this).add("LAB",msg);
            }finally{
                labBusy=false;
            }
        }).start();
    ...
    private String shortMsg(Exception e){String s=e.getMessage();if(s==null)s=e.getClass().getSimpleName();return s.length()>700?s.substring(0,700):s;}
