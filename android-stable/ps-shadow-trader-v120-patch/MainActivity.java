...
public class MainActivity extends Activity {
    ...
    private void buildShell(){
        ...
        TextView sub=text("Adaptive Multify engine + autonomous market learning",14,MUTED,false);sub.setPadding(0,dp(2),0,dp(14));page.addView(sub);
    ...
    private void renderAuto(){
        ...
        strategy.addView(sectionTitle("Adaptive strategy ensemble","Weights change from actual shadow outcomes"));
        ...
        learn.addView(sectionTitle("Self-learning & logging","Persistent on-device feedback loop"));
        learn.addView(text("Each closed shadow trade stores its entry context, side, prices, net outcome and reason. Profitable contexts reinforce the strategies that supported them; losing contexts reduce those weights and increase the learned turnover penalty. EOD replay summarizes broader performance so one bad NSE day does not cause a large model change.",13,NAVY,false));
    ...
    private void renderLab(){
        ...
        head.addView(text("The research engine continuously replays price/volume patterns and ranks NSE CASH/EQ stocks. A 50% target is an experimental forecast target, not a guaranteed outcome.",13,NAVY,false));
        ...
        head.addView(metric("Scanner","Automatic off-market rotating NSE scan"));
        head.addView(metric("Validation","Historical replay + relative-strength/volatility filters"));
    ...
    private void authenticate(){
        ...
        new Thread(()->{
            try{
                ...
                runOnUiThread(()->{
                    Toast.makeText(this,!tt.isEmpty()?"TOTP saved. Adaptive automation started.":"Groww authenticated. Adaptive automation started.",Toast.LENGTH_LONG).show();
