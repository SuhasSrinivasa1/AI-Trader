...
public final class AdaptiveModel {
    ...
    public AdaptiveModel(Context c){
        p=c.getSharedPreferences("adaptive_model",Context.MODE_PRIVATE);
    ...
    public synchronized void learnOnClose(String symbol,String side,double entry,double exit,double netPnlPct,String reason){
        ...
        try{
            ...
            int total=p.getInt("trades_total",0)+1,wins=p.getInt("wins",0)+(netPnlPct>0?1:0);
    ...
    public double turnoverPenalty(){return clamp(p.getFloat("turnover_penalty",1.0f),0.65,2.5);}
    ...
    public double dynamicConfidenceFloor(double volatilityPct){
        double base=trades()<8?0.68:0.60;
        if(hitRate()>0&&hitRate()<0.45)base+=0.08;
        if(recentReward()<-.002)base+=0.05;
        if(volatilityPct>.018)base+=0.03;
        return clamp(base,0.55,0.82);
    }
    ...
    public String summary(){
        ...
        s.append("Trades ").append(trades()).append(" • hit ");
        s.append(trades()==0?"—":String.format(Locale.US,"%.0f%%",hitRate()*100));
        s.append(" • turnover penalty ").append(String.format(Locale.US,"%.2fx",turnoverPenalty()));
        s.append("\nLeading weights: ");
