...
public class AutomationService extends Service {
    ...
    private void cycle(){
        ...
        notifyStatus("FINAL • "+(market?"2s live learner":"replay/training")+" • "+(ts.side==Side.FLAT?"FLAT":ts.symbol+" "+ts.side)+" • Net ₹"+String.format(Locale.US,"%.0f",ts.pnl())+" • G"+champion.generation());
    }
    ...
    private void runOffMarket(String token,android.content.SharedPreferences state,long now,int hm){
        ...
        if((force||(hm>=1545&&hm<=2359))&&eodDay!=day){
            ...
            state.edit().putLong("last_eod_day",day).putBoolean("force_eod",false)
                    ...
                    .putString("champion_summary",champion.summary())
                    .putString("tournament_status",tournament).apply();

            new EventStore(this).add("EOD","Training + champion replay complete • generation "+champion.generation());
        }
    ...
    private void createChannel(){
        if(Build.VERSION.SDK_INT>=26){
            ...
            c.setDescription("Live learning, champion replay and shadow decisions");
