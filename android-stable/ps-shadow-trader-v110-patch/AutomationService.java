...
public class AutomationService extends Service {
    ...
    private void cycle(){
        SecureStore secure=new SecureStore(this);String token=secure.get("token");android.content.SharedPreferences state=getSharedPreferences("state",MODE_PRIVATE);
        if(token.isEmpty()){state.edit().putString("engine_status","Waiting for Groww token").apply();notifyStatus("Waiting for daily Groww token");return;}
        long now=System.currentTimeMillis();int hm=hm();boolean weekday=weekday();boolean market=weekday&&hm>=915&&hm<=1520;
        try{
            long lastAuth=state.getLong("last_auth_check",0);if(now-lastAuth>45*60000L){new GrowwClient().user(token);state.edit().putLong("last_auth_check",now).putBoolean("token_ok",true).putString("engine_status","Automation running").apply();}
        }catch(Exception e){state.edit().putBoolean("token_ok",false).putString("engine_status","Token needs refresh").apply();new EventStore(this).add("AUTH","Groww token check failed — authenticate a fresh token");notifyStatus("Groww token needs refresh");return;}
        if(market)runIntraday(token,state,now,hm);
        else runOffMarket(token,state,now,hm);
    ...
    private void runIntraday(String token,android.content.SharedPreferences state,long now,int hm){
        ...
        try{
            long end=now/1000L,start=end-3L*86400L;List<Candle> c=new GrowwClient().candles(token,symbol,start,end,"5minute");List<Candle> n=new GrowwClient().candles(token,"NIFTY",start,end,"5minute");double nr=seriesReturn(n);double px=api.ltp(token,symbol);ModelConfig cfg=new ModelConfig(this);Signal sig=new StrategyEngine().evaluate(c,nr,cfg.entryThreshold());String action=new TradeSimulator().apply(ts,symbol,sig,px,cfg.reversalThreshold());store.save(ts);
