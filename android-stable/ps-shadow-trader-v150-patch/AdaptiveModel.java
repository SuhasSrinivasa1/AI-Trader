...
public final class AdaptiveModel {
    ...
    public synchronized void rememberEntry(String symbol,String side,double price,Map<String,Double>features,double confidence,double edgePct){
        try{
            JSONObject o=new JSONObject().put("symbol",symbol).put("side",side).put("price",price).put("ts",System.currentTimeMillis()).put("confidence",confidence).put("edge",edgePct).put("max_seen",price).put("min_seen",price);
    ...
    public synchronized void observeOpen(String symbol,double price){
        if(symbol==null||symbol.isEmpty()||price<=0||Double.isNaN(price))return;
        try{
            JSONObject o=new JSONObject(p.getString("open_context","{}"));
            if(!symbol.equalsIgnoreCase(o.optString("symbol","")))return;
            double max=o.optDouble("max_seen",o.optDouble("price",price));
            double min=o.optDouble("min_seen",o.optDouble("price",price));
            o.put("max_seen",Math.max(max,price)).put("min_seen",Math.min(min,price)).put("last_seen",price).put("last_seen_ts",System.currentTimeMillis());
            p.edit().putString("open_context",o.toString()).apply();
        }catch(Exception ignored){}
    }
    ...
    public synchronized void learnOnClose(String symbol,String side,double entry,double exit,double netPnlPct,String reason){
        ...
        try{
            ...
            double maxSeen=ctx==null?entry:ctx.optDouble("max_seen",entry);
            double minSeen=ctx==null?entry:ctx.optDouble("min_seen",entry);
            double mfe=0,mae=0;
            if(entry>0){
                if("SHORT".equals(side)){
                    mfe=Math.max(0,entry/Math.max(.0001,minSeen)-1);
                    mae=Math.max(0,maxSeen/entry-1);
                }else{
                    mfe=Math.max(0,maxSeen/entry-1);
                    mae=Math.max(0,1-minSeen/entry);
                }
            }
            double capture=mfe<=0?0:clamp(netPnlPct/mfe,0,1.5);
            String postmortem=postmortem(ctx,side,netPnlPct,reason);

            JSONArray trades=new JSONArray(p.getString("trades","[]"));
            JSONObject t=new JSONObject().put("ts",System.currentTimeMillis()).put("symbol",symbol).put("side",side).put("entry",entry).put("exit",exit)
                    .put("net_pct",netPnlPct).put("reason",reason).put("result",netPnlPct>0?"WIN":"LOSS")
                    .put("mfe",mfe).put("mae",mae).put("capture",capture).put("postmortem",postmortem);
    ...
    private String postmortem(JSONObject ctx,String side,double net,String exitReason){
        if(net>0)return "WIN";
        if(ctx==null)return "UNEXPLAINED";
        JSONObject f=ctx.optJSONObject("features");
        if(f==null)return "UNEXPLAINED";
        double breakout=f.optDouble("breakout",0),volume=f.optDouble("volume",0),relative=f.optDouble("relative",0),
                micro=f.optDouble("micro",0),accel=f.optDouble("accel",0),vwap=f.optDouble("vwap",0);
        boolean lng="LONG".equals(side);
        if(Math.abs(breakout)>.75)return "FALSE BREAKOUT / BREAKDOWN";
        if(Math.abs(volume)<.12)return "WEAK PARTICIPATION";
        if((lng&&relative<-.20)||(!lng&&relative>.20))return "MARKET RELATIVE WEAKNESS";
        if((lng&&micro<-.15)||(!lng&&micro>.15)||(lng&&accel<-.20)||(!lng&&accel>.20))return "MOMENTUM REVERSAL";
        if((lng&&vwap<-.35)||(!lng&&vwap>.35))return "VWAP STRUCTURE FAILURE";
        if(exitReason!=null&&exitReason.toLowerCase(Locale.ROOT).contains("rotation"))return "BETTER ALTERNATIVE / ROTATION";
        return "UNEXPLAINED / NORMAL VARIANCE";
    }
