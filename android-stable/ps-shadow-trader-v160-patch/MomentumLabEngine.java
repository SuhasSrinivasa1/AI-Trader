...
public final class MomentumLabEngine {
    ...
    public String scanNextBatch(String token)throws Exception{
        ...
        for(int k=0;k<BATCH&&k<universe.size();k++){
            ...
            try{
                ...
                Guard guard=guard(d,sector);
                LongHorizonChampionStore championStore=new LongHorizonChampionStore(c);
                int generation=championStore.generation();
                boolean validated=generation>0&&champion.holdoutSignals>=6&&champion.holdoutPrecision>=.25;

                double raw=LongHorizonFeatures.probability(f,champion);
                double blended=clamp(raw*.60+replay.hitRate*.25+Math.min(.08,replay.matches*.008)+Math.min(.07,replay.medianPeak*.10),.01,.99);
                double modelScore=calibrate(p,blended);
                modelScore=clamp(modelScore-guard.penalty,.01,validated?.94:.79);
                double threshold=Math.max(champion.threshold,validated?.72:.76);

                double last=d.get(d.size()-1).c;
                if(last<=0)continue;

                boolean replayOk=replay.matches>=8
                        && replay.hitRate>=(validated?.50:.60)
                        && replay.medianPeak>=(validated?.45:.50);
                boolean extensionOk=!guard.high||replay.hitRate>=.80;
                boolean publish=modelScore>=threshold&&replayOk&&extensionOk;

                updateExistingPrediction(p,symbol,d);
                if(publish){
                    Candidate x=new Candidate();
                    x.symbol=symbol;x.price=last;x.target=last*1.5;x.confidence=modelScore;x.rawConfidence=raw;
                    ...
                    x.sector=sector;x.generation=generation;x.champion=champion.id;
                    x.matches=replay.matches;x.replayHitRate=replay.hitRate;x.replayMedianPeak=replay.medianPeak;
                    x.validated=validated;x.extensionRisk=guard.level;x.extensionPct=guard.extensionPct;x.extensionAtr=guard.extensionAtr;
                    x.circuitLike=guard.circuitLike;x.guardPenalty=guard.penalty;
                    x.reason=reason(f,replay,nifty20,sector,sectorMean,champion,guard,validated);
                    if(upsertCurrentSnapshotLocked(p,x))added++;
                }else if(modelScore>=.45){
                    storeControl(p,symbol,last,modelScore,sector,champion.id);
                    controls++;
                }
            ...
            for(int i=0;i<a.length();i++){
                ...
                if(o.optString("symbol").equals(x.symbol)){
                    ...
                    o.put("lastObserved",x.price)
                            ...
                            .put("latestValidated",x.validated)
                            .put("latestExtensionRisk",x.extensionRisk)
    ...
    private ReplayEvidence localReplay(List<Candle>d,Map<String,Double>current){
        ...
        for(int i=70;i<n-1;i+=2){
            ...
            if(base<=0)continue;
            for(int j=i+1;j<n&&d.get(j).ts<=horizon;j++)peak=Math.max(peak,d.get(j).h/base-1);
    ...
    private JSONObject toJson(Candidate x)throws Exception{
        ...
        return new JSONObject().put("symbol",x.symbol).put("price",x.price).put("target",x.target)
                .put("confidence",x.confidence).put("rawConfidence",x.rawConfidence).put("scoreType","MODEL_SCORE")
                ...
                .put("validated",x.validated).put("extensionRisk",x.extensionRisk)
                .put("extensionPct",x.extensionPct).put("extensionAtr",x.extensionAtr)
                .put("circuitLike",x.circuitLike).put("guardPenalty",x.guardPenalty)
                .put("horizon","30_calendar_days")
    ...
    private String reason(Map<String,Double>f,ReplayEvidence r,double nifty20,String sector,double sectorMean,LongHorizonChampionStore.Profile p,Guard g,boolean validated){
        return String.format(Locale.US,
                "%s • 30-calendar-day replay +50%% %.0f%% (%d matches) • median peak %.1f%% • extension %s (%.1f%% / %.1f ATR) • circuit-like sessions %d • 20d relative vs NIFTY %.1f%% • sector %s • breakout %.2f • volume %.2f",
                validated?"VALIDATED MODEL":"UNVALIDATED MODEL SCORE",r.hitRate*100,r.matches,r.medianPeak*100,g.level,g.extensionPct*100,g.extensionAtr,g.circuitLike,
                f.get("relative")/6*100,sector,f.get("breakout"),f.get("volume"));
    }

    private Guard guard(List<Candle>d,String sector){
        Guard g=new Guard();int n=d.size();if(n<25)return g;
        double last=d.get(n-1).c,sma=0;
        for(int i=n-20;i<n;i++)sma+=d.get(i).c;
        sma/=20.0;
        double atr=0;
        for(int i=n-14;i<n;i++){
            Candle z=d.get(i),prev=d.get(i-1);
            double tr=Math.max(z.h-z.l,Math.max(Math.abs(z.h-prev.c),Math.abs(z.l-prev.c)));
            atr+=tr;
        }
        atr/=14.0;
        g.extensionPct=sma<=0?0:last/sma-1;
        g.extensionAtr=atr<=0?0:(last-sma)/atr;
        double r5=ret(d,5),r20=ret(d,20);
        for(int i=Math.max(1,n-5);i<n;i++){
            Candle z=d.get(i),prev=d.get(i-1);
            double dayRet=prev.c<=0?0:z.c/prev.c-1;
            boolean nearHigh=z.h>0&&(z.h-z.c)/z.h<=.003;
            if(nearHigh&&dayRet>=.045)g.circuitLike++;
        }
        boolean unknown=sector==null||sector.trim().isEmpty()||"UNKNOWN".equalsIgnoreCase(sector);
        g.high=g.extensionAtr>=4.0||g.extensionPct>=.28||r5>=.25||r20>=.50||g.circuitLike>=2;
        boolean elevated=g.high||g.extensionAtr>=3.0||g.extensionPct>=.18||r5>=.15||r20>=.32||g.circuitLike>=1;
        g.level=g.high?"HIGH":(elevated?"ELEVATED":"NORMAL");
        g.penalty=(g.high?.14:(elevated?.07:0))+(unknown?.04:0);
        return g;
    }
    ...
    private static class Candidate{String symbol,reason,sector,champion,extensionRisk;double price,target,confidence,rawConfidence,maxSeen,replayHitRate,replayMedianPeak,extensionPct,extensionAtr,guardPenalty;long created,lastEvaluated,expiry;int matches,generation,circuitLike;boolean validated;Map<String,Double>features;}
    private static class ReplayEvidence{int matches;double hitRate,medianPeak;}
    private static class Guard{String level="NORMAL";double extensionPct,extensionAtr,penalty;int circuitLike;boolean high;}
