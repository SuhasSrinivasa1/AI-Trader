...
public class MainActivity extends Activity {
    ...
    private void buildShell(){
        ...
        TextView sub=text("v1.6 • Multify professional terminal",13,MUTED,false);sub.setPadding(0,dp(2),0,dp(14));page.addView(sub);
    ...
    private void buildAuthCard(){
        ...
        manualToken=secretField("Manual access token (optional)");manualToken.setText(secure.get("token"));
        LinearLayout.LayoutParams p3=new LinearLayout.LayoutParams(-1,dp(48));p3.topMargin=dp(7);card.addView(manualToken,p3);

        Button auth=button("SAVE & START",BLUE,Color.WHITE);auth.setOnClickListener(v->authenticate());
        LinearLayout.LayoutParams bp=new LinearLayout.LayoutParams(-1,dp(46));bp.topMargin=dp(9);card.addView(auth,bp);
    ...
    private void buildTabs(){
        ...
        tabAuto=button("MULTIFY",NAVY,Color.WHITE);tabLab=button("30-DAY LAB",Color.TRANSPARENT,NAVY);
    ...
    private void renderAuto(){
        ...
        setup.addView(sectionTitle("EXECUTION","Trading configuration"));
        ...
        Button edit=button("EDIT SETUP",Color.rgb(238,242,249),NAVY);
    ...
    private View candidateCard(JSONObject o,boolean closed){
        ...
        TextView score=text(String.format(Locale.US,"%.0f%%",originalScore*100),15,closed?(o.optBoolean("success")?GREEN:RED):BLUE,true);
        ...
        if(closed){
            ...
            c.addView(text("30-DAY CANDIDATE • ORIGINAL SNAPSHOT LOCKED",11,GREEN,true));
        }
        ...
        if(o.has("champion")){
            c.addView(metric("Original model snapshot","Champion "+o.optString("champion")+" • generation "+o.optInt("generation")+" • score "+String.format(Locale.US,"%.0f/100",originalScore*100)));
        }
        ...
        if(closed){
            ...
            if(o.has("latestModelScore"))c.addView(metric("Latest model score",String.format(Locale.US,"%.0f/100",o.optDouble("latestModelScore")*100)+" • original score remains locked above"));
    ...
    private void openLiveSetup(){
        ...
        TextView note=text("₹0 – ₹1,00,000 • saved for a future explicitly armed live mode; shadow size stays ₹10,000.",11,MUTED,false);box.addView(note);
