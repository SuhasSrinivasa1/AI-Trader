...
public final class MomentumLabEngine {
    ...
    public String scanNextBatch(String token)throws Exception{
        ...
        long end=System.currentTimeMillis()/1000L,start=end-620L*86400L;
        ...
        int checked=0,added=0,controls=0;
        ...
        for(int k=0;k<BATCH&&k<universe.size();k++){
            ...
            try{
                ...
                if(confidence>=threshold&&replay.matches>=2){
                    ...
                    x.created=System.currentTimeMillis();x.expiry=x.created+30L*DAY;x.maxSeen=last;
        ...
        String msg="30-Day Lab scanned "+checked+" NSE equities • "+added+" published • "+controls+" near-miss controls • coverage "+newCursor+"/"+universe.size()+" • cycle "+cycle+" • examples "+dataset.size();
        ...
        try{
            ...
            for(int i=0;i<a.length();i++){
                ...
                if(o.optString("symbol").equals(x.symbol)){
                    found=true;
                    o.put("lastObserved",x.price).put("maxSeen",Math.max(o.optDouble("maxSeen",o.optDouble("price")),x.price));
                }
                b.put(o);
            }
        ...
        try{
            ...
            for(int i=0;i<a.length();i++){
                ...
                if(o.optString("symbol").equals(symbol)){
                    ...
                    o.put("maxSeen",max).put("lastObserved",d.get(d.size()-1).c);
        ...
        try{
            ...
            for(int i=0;i<a.length();i++){
                ...
                o.put("maxSeen",out.maxPrice).put("minSeen",out.minPrice).put("closed",now)
                        .put("success",out.maxGain>=.50).put("hit30",out.maxGain>=.30)
    ...
    private JSONObject toJson(Candidate x)throws Exception{
        return new JSONObject().put("symbol",x.symbol).put("price",x.price).put("target",x.target)
                ...
                .put("created",x.created).put("expiry",x.expiry).put("maxSeen",x.maxSeen)
    ...
    private static class Candidate{String symbol,reason,sector,champion;double price,target,confidence,rawConfidence,maxSeen,replayHitRate,replayMedianPeak;long created,expiry;int matches,generation;}
