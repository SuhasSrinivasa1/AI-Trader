...
public final class ThirtyDayTournamentEngine {
    ...
    public synchronized String run(){
        ...
        for(Genome g:genomes){
            ...
            double combined=.25*tr.score+.75*va.score;
            if(va.signals<3)combined-=.25;
        ...
        double baseScore=.25*evaluate(base,train).score+.75*baseVal.score;

        boolean promote=best!=null&&!best.id.equals(incumbent.id)&&bestM.signals>=3&&bestM.precision>=.18&&bestCombined>baseScore+.03;
        String summary;
        if(promote){
            ...
            summary="PROMOTED "+best.id+" • holdout precision "+pct(bestM.precision)+" • recall "+pct(bestM.recall)+" • signals "+bestM.signals+" • examples "+all.size();
