...
public final class ThirtyDayDataset {
    ...
    public static final class Example {
        public long ts; public boolean hit50,hit30; public double futurePeak,futureDrawdown; public final Map<String,Double>f=new LinkedHashMap<>();
    }

    public synchronized int size(){try{return new JSONArray(p.getString("examples","[]")).length();}catch(Exception e){return 0;}}

    public synchronized void addFromHistory(List<Models.Candle>d,double[] nifty20,String sector,double sectorMean){
        if(d==null||d.size()<130)return;
        ArrayList<JSONObject> positives=new ArrayList<>(),negatives=new ArrayList<>();
        int n=d.size();
        for(int i=70;i<n-31;i+=3){
            ...
            for(int j=i+1;j<=Math.min(n-1,i+30);j++){peak=Math.max(peak,d.get(j).h/base-1);low=Math.min(low,d.get(j).l);dd=Math.max(dd,1-low/base);}
            try{
                JSONObject o=new JSONObject().put("ts",d.get(i).ts).put("hit50",peak>=.50).put("hit30",peak>=.30).put("peak",peak).put("dd",dd);
    ...
    private synchronized void append(List<JSONObject> add){
        try{
            ...
            for(int i=0;i<old.length();i++)all.add(old.getJSONObject(i));
            all.addAll(add);
    ...
    public synchronized List<Example> all(){
        ...
        try{
            ...
            for(int i=0;i<a.length();i++){
                JSONObject o=a.getJSONObject(i);Example e=new Example();e.ts=o.optLong("ts");e.hit50=o.optBoolean("hit50");e.hit30=o.optBoolean("hit30");e.futurePeak=o.optDouble("peak");e.futureDrawdown=o.optDouble("dd");
