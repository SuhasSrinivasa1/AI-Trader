...
public class MainActivity extends Activity {
    ...
    private LinearLayout body,recs;private TextView successRate,successSub,market,status,scanMeta,active;private Switch liveSwitch;
    private LearningStore learning;

    @Override protected void onCreate(Bundle b){super.onCreate(b);getWindow().setStatusBarColor(BG);getWindow().setNavigationBarColor(BG);learning=new LearningStore(this);setContentView(buildUi());requestNotificationPermission();startScanner();ui.post(refreshLoop);}
    ...
    private View buildUi(){
        ...
        TextView h=txt("TOP 3 • NEXT 30 MINUTES",18,TEXT,true);body.addView(h);TextView note=txt("BUY appears only when hard gates + learned quality threshold pass. Same stock may remain ranked on consecutive scans.",12,MUTED,false);body.addView(note);body.addView(space(8));recs=new LinearLayout(this);recs.setOrientation(LinearLayout.VERTICAL);body.addView(recs);
        body.addView(space(14));
        LinearLayout learn=card();learn.addView(txt("SELF-LEARNING RULE",12,ACCENT,true));learn.addView(txt("Every recommendation is replayed for 30 minutes. Target-before-stop = success; stop/timeout = failure; same-candle target+stop is excluded as ambiguous. Learning may tighten selection, but cannot change the 0.50% net target, 30-minute time stop, liquidity gates, or protection rules.",12,MUTED,false));body.addView(learn);
        body.addView(space(8));TextView footer=txt("v1.0.0 • standalone package com.suhas.nseunifiedscanner • Multyfi AutoBuy is untouched",10,MUTED,false);footer.setGravity(Gravity.CENTER);body.addView(footer);
        return scroll;
    }

    private final Runnable refreshLoop=new Runnable(){@Override public void run(){render();ui.postDelayed(this,1000);}};
    private void render(){
        LearningStore.Stats s=learning.stats();if(s.n==0){successRate.setText("—");successRate.setTextColor(TEXT);successSub.setText("No resolved scanner recommendations yet • goal ≥80%, never guaranteed");}else{successRate.setText(String.format(Locale.US,"%.1f%%",s.rate));successRate.setTextColor(s.rate>=80?GREEN:s.rate>=70?AMBER:RED);successSub.setText("Last "+s.n+" resolved recommendations • "+s.wins+" target hits • executed sample "+s.executedN);}
